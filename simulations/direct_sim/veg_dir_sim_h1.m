...
%%% STEP SIZE CONSTANTS (Important ones: c, w_plus) %%%

c = 2;  % advective constant
w_plus_s = 4;
d_w = 0.1;
w_plus_e = w_plus_s; % if you want multiple w_plus values, have range
...
% we set most of these to zero for the time being, will be checked
...
F = @(t, V) veg_model_f(t, V, D1, D2, N, dx, c, w_plus);
...
b = -(w_plus) * (tanh(const*(x - floor(L/4))) + 1) + 2*w_plus;
w = (w_plus / 2) * (tanh(const*(x - floor(L/4))) + 1);
...
% title(['Space $$x$$ vs. Water $$w$$, $$t = 0$$'], 'FontSize', fontsize)
...
title(['Space $$x$$ vs. Biomass $$b$$ with $$c =$$ ' num2str(c) ...
        ', $$w_+ =$$ ' num2str(w_plus) ', time $$t = 0$$'], ...
            'FontSize', fontsize)
% title(['Space $$x$$ vs. Biomass $$b$$, $$t = 0$$'], 'FontSize', fontsize)
...
title(['Space $$x$$ vs. Biomass $$b$$'], 'FontSize', fontsize)
subplot(2, 1, 1)
title(['Space $$x$$ vs. Water $$w$$ with Front speed $$s =$$ ' num2str(s)], ...
        'FontSize', fontsize)
