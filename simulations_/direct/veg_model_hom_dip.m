...
s = 0.9;
c = 2;
L_wid = 5;          % how spaced out mid is (2*L_wid)

% Other Constants 

b_plus = (theta * sqrt(s + c) + sqrt((theta^2 - 4*s)*(s+c))) / (2*s);
w_plus = 1 / b_plus;
...
t_eps = 0.05;    % threshold for front when calculating speed s
...
% Opposite interface as in veg_model_hom.m

w = (w_plus / 2) * (tanh(const*(x - floor(L/2 - L_wid)))) - ...
        (w_plus / 2) * (tanh(const*(x - floor(L/2 + L_wid))));
b = -1*((b_plus / 2) * tanh(const*(x - floor(L/2 - L_wid)))) + ...
     (((b_plus / 2) * tanh(const*(x - floor(L/2 + L_wid))))) + ...
     b_plus;
...
title(['Space $$x$$ vs. Water $$w$$ at time $$t = 0$$'], ...
        'FontSize', fontsize)

subplot(2, 1, 2)
plot(x, b, 'g', 'LineWidth', linewidth)
axis([0 L 0 4]) % Might have to increase y-axis
xt = get(gca, 'XTick');
set(gca, 'FontSize', tick_fontsize) 
xlabel('Space $$x$$', 'FontSize', fontsize)
ylabel('Biomass $$b$$', 'FontSize', fontsize)
title(['Space $$x$$ vs. Biomass $$b$$ at time $$t = 0$$'], ...
        'FontSize', fontsize)
